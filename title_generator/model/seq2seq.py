...
class Seq2SeqTitleGenerator(pl.LightningModule):
    ...
    def configure_optimizers(self):
        if hasattr(torch.optim, self.optimizer_name):
            ...
            optimizer = getattr(transformers.optimization, self.optimizer_name)(
                self.parameters(), **self.optimizer_args
            )
        else:
            raise AttributeError(
                f"{self.optimizer_name} is not a valid name, searched torch.optim and transformers.optimization."
            )
        return optimizer
